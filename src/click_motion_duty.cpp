...
private:
  // 移動ステップ。Pがつくと並行かき分け動作  
  enum class Step { WAIT_FOR_INPUT, X, Y, Z, R_X, R_R, WAIT, R_Y,R_S, PWAIT1,PX, PY, PZ, PR_X, PR_R, PWAIT2, PR_Y,PR_S, DONE };
  Step current_step_;
    ...
    switch (current_step_) {
      ...
      case Step::Y:
        if (std::abs(dy_approach) < threshold) {
          ...
          raking_start_y_ = current_y; // ★current_yを必ず保存
          RCLCPP_INFO(this->get_logger(), "raking_start_y_: %.6f, current_y: %.6f", raking_start_y_, current_y); // ★保存値をログ出力
     ...
     case Step::WAIT:
      ...
      case Step::R_Y:
      {
        ...
        RCLCPP_INFO(this->get_logger(), "[R_Y] raking_start_y_: %.6f, current_y: %.6f, return_dy: %.6f", raking_start_y_, current_y, return_dy); // ★判定前に出力
      ...
      case Step::R_S:
      {
        ...
        if (dist_to_start < threshold) {
          publish_stop();
          RCLCPP_INFO(this->get_logger(), "初期位置へ移動完了.PWAIT1へ");
          current_step_ = Step::PWAIT1;
          wait_started_ = true;
          return;
        }
        double return_mag = dist_to_start;
      ...
      if (wait_started_) {
        if (raking_trigger_){
          RCLCPP_INFO(this->get_logger(), "raking_trigger 受信 → PX");
          current_step_ = Step::PX;
          raking_trigger_ = false;
          wait_started_ = false;
          return;
        }
     ...
     case Step::PWAIT2:
      ...
      {
        double return_dy = initial_y - current_y;
        RCLCPP_INFO(this->get_logger(), "[PR_Y] raking_start_y_: %.6f, current_y: %.6f, return_dy: %.6f", raking_start_y_, current_y, return_dy); // ★判定前に出力
        if (std::abs(return_dy) < threshold) {
          publish_stop();
          RCLCPP_INFO(this->get_logger(), "Y軸後退完了.初期位置へ");
          current_step_ = Step::PR_S;
          return;
        }
        twist.twist.linear.y = scale * (return_dy > 0 ? 1 : -1);
      }
